#Author: Roman Eduard-Emanuel
#Date: ~02.2021
#Description: the program looks inside the .pdf file for symbols specific to pass or fail marks
#             generated by the calibration company


#Importing libraries for the .pdf files read and tkinter GUI
import re
import os
import PyPDF2
...
#function for opening files directories
def open_dir():
...
#function for renaming the file according to the SN
#for an easier database storage
def redenumire():
...
#function for the verification and for opening a side window 
#for visualising the files that contain the specific symbols
def verificare():
    
    #define the GUI window frame and size
    ...
    for pdfFile in os.listdir(dir):
        if ".pdf" in pdfFile:
            ...
            #This section is parsing the document according to FirmaX
            #The tool will be update for other companies as well
            
            if text1.get().lower() == 'FirmaX':
            ...
            if button2["text"].lower() == "nu":
                ...
                try:
                    os.rename(dir + "/" + pdfFile, dir + "/" + page[page.index("indiciu specific FirmaX") + 1] + '.pdf') #specific location of the SN for FirmaX .pdf layout
                except FileExistsError:
                    os.rename(dir + "/" + pdfFile, dir + "/" + page[page.index("indiciu specific FirmaX") + 1] + ' - copy.pdf') #error handling for same filename
                    
                
# configuration of the GUI (text, labels, buttons, frame size, etc.)
...
label2 = tk.Label(master = fr1, text = "Indicii de fail specifice firmeiX")
